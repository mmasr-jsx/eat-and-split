...
function Friend({ friend, onSelectFriend, selectedFriend }) {
  ...
  return (
    <li className={isSelected ? "selected" : ""}>
      ...
      {friend.balance < 0 && (
        <p className="red">
          We owe this light creature {friend.name} {Math.abs(friend.balance)}€
        </p>
      )}
      {friend.balance > 0 && (
        <p className="green">
          This little *** owe's us {friend.name} {Math.abs(friend.balance)}€
        </p>
      )}
      {friend.balance === 0 && (
  ...
  return (
    <form className="form-add-friend" onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        value={image}
        onChange={(e) => setImage((curImg) => e.target.value)}
      />
